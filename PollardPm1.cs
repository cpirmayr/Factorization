...
public class PollardPMinus1
{
  ...
  /// <param name="baseValue">Basis (default: 2).</param>
  public PollardPMinus1(BigInteger bound = default, int gcdInterval = 20, BigInteger baseValue = default)
  {
    Bound = bound == 0 ? BigInteger.Zero : bound;
    GcdInterval = gcdInterval;
    Base = baseValue == 0 ? 2 : baseValue;
  }
  ...
  {
    List<long> primes = SieveOfEratosthenes(bound);
    BigInteger a = Base;
    int i = 0;
